...
ggplot(data = Table_prediction_rcs_FeNO, aes(x = FeNO_baseline_ppb_imputated, y = Prediction, linetype = Any_severe_attack_previous_12m_0no_1yes_imputated)) +
  geom_smooth(color = "darkred", size = 0.8) +  # Set all lines to black with distinct patterns
  xlab("FeNO (ppb)") +
  ylab("Estimated Annualised Severe Asthma Attack Rate") +
  scale_linetype_manual(
...
ggplot(data = Table_prediction_rcs_BEC, aes(x = Blood_Eos_baseline_x10_9_cells_per_L_imputated, y = Prediction, linetype = Any_severe_attack_previous_12m_0no_1yes_imputated)) +
  geom_smooth(color = "darkred", size = 0.8) +  # Set all lines to black with distinct patterns
  xlab("BEC (x10^9 cells/L)") +
  ylab("Estimated annualised Severe Asthma Attack Rate") +
  scale_linetype_manual(
...
ggplot(data = Table_prediction_rcs, aes(x = .data[[variable_1]], y = Prediction, colour = .data[[variable_2]])) +
  geom_smooth() +
  #xlab(expression(bold(paste("Blood Eosinophils (", bold(10^{9}), "cells/L)")))) +
  xlab("BEC (x10^9 cells/L)") +
  ylab("Estimated annualised Severe Asthma Attack Rate") +
  scale_color_manual(values = c("darkgreen", "gold", "darkorange", "darkred"), name = "FEV1 \n(% of predicted)") +
...
Spline_BEC_X_FEV1<-ggplot(data = Table_prediction_rcs, aes(x = .data[[variable_1]], y = Prediction, linetype = .data[[variable_2]])) +
  geom_smooth(color = "darkblue") +  # Set all lines to black
  xlab("BEC (x10^9 cells/L)") +
  ylab("Estimated annualised Severe Asthma Attack Rate") +
...
ggplot(data = Table_prediction_rcs, aes(x = .data[[variable_1]], y = Prediction, colour = .data[[variable_2]])) +
  ...
  xlab('FeNO (ppb)') +
  ylab("Estimated annualised Severe Asthma Attack Rate") +
  scale_color_manual(labels = c("Yes","No"),values=c("darkred","darkgreen"),name = "Any attack past 12m") +
  scale_x_continuous(trans='log2',breaks = c(0,10,25,50,100,200)) +
  scale_y_continuous(breaks = c(0.5, 1, 1.5)) +  # Custom y-axis ticks
...
Spline_FeNO_X_Attack<-ggplot(data = Table_prediction_rcs, aes(x = FeNO_baseline_ppb_imputated, y = Prediction, linetype = Any_severe_attack_previous_12m_0no_1yes_imputated)) +
  geom_smooth(color = "darkred", size = 0.8) +  # Set all lines to black with distinct patterns
  xlab("FeNO (ppb)") +
  ylab("Estimated Annualised Severe Asthma Attack Rate") +
  scale_linetype_manual(
...
Spline_BEC_X_Attack<-ggplot(data = Table_prediction_rcs, aes(x = Blood_Eos_baseline_x10_9_cells_per_L_imputated, y = Prediction, linetype = Any_severe_attack_previous_12m_0no_1yes_imputated)) +
  geom_smooth(color = "darkred", size = 0.8) +  # Set all lines to black with distinct patterns
  xlab("BEC (x10^9 cells/L)") +
  ylab("Estimated annualised Severe Asthma Attack Rate") +
  scale_linetype_manual(
...
ggplot(data = Table_prediction_rcs, aes(x = .data[[variable_1]], y = Prediction, colour = .data[[variable_2]])) +
  ...
  xlab(paste("FeNO (ppb)")) +
  ylab("Estimated annualised Severe Asthma Attack Rate") +
  scale_color_manual(values=c("darkgreen","gold","darkorange", "darkred"),name = "Treatment step") +
  scale_x_continuous(trans='log2',breaks = c(0,10,25,50,100,200)) +
...
Spline_FeNO_X_GINA<-ggplot(data = Table_prediction_rcs, aes(x = .data[[variable_1]], y = Prediction, linetype = .data[[variable_2]])) +
  geom_smooth(color = "#6A0DAD") +  # Set all lines to black
  xlab('FeNO (ppb)') +
  ylab("Estimated annualised Severe Asthma Attack Rate") +
  scale_linetype_manual(values = c("solid", "dashed", "dotted", "dotdash"), 
                        name = "GINA \nTreatment step",
...
ggplot(data = Table_prediction_rcs, aes(x = .data[[variable_1]], y = Prediction, colour = .data[[variable_2]])) +
  ...
  xlab(paste("FeNO (ppb)")) +
  ylab("Estimated annualised Severe Asthma Attack Rate") +
  scale_color_manual(values=c("darkgreen","gold","orange","darkred"),name = "ICS dose") +
  scale_x_continuous(trans='log2',breaks = c(0,10,25,50,100,200)) +
  #coord_cartesian(xlim =c(10, 100), ylim = c(0, 2)) +
  theme(legend.position = c(0.2, 0.8)) +
  theme(
...
Spline_FeNO_X_ICS<-ggplot(data = Table_prediction_rcs, aes(x = .data[[variable_1]], y = Prediction, linetype = .data[[variable_2]])) +
  geom_smooth(color = "#D55E00") +  # Set all lines to black
  xlab('FeNO (ppb)') +
  ylab("Estimated annualised Severe Asthma Attack Rate") +
  scale_linetype_manual(values = c("solid", "dashed", "dotted", "dotdash"), 
                        name = "ICS Dose",
...
unique(Data_Oracle_only_with_baseline_lung$Enrolled_Trial_name)
...
#======================================================================================================================================
# Spline curve FENO X Attack history according to the GINA STEP
variable_1<-"FeNO_baseline_ppb_imputated"
variable_2<-"Any_severe_attack_previous_12m_0no_1yes_imputated"
Data_Oracle_only_with_asthma_history_yes_or_no$Treatment_step_1and2
plots_by_treatment <- list()
# Loop through each level of Treatment_step_1_2
for (treatment in unique(Data_Oracle_only_with_asthma_history_yes_or_no$Treatment_step_1and2)) {
  # Filter the data for the current subgroup
  subgroup_data <- subset(Data_Oracle_only_with_asthma_history_yes_or_no, Treatment_step_1and2 == treatment)
  
  # Initialize a matrix to store predictions across imputations
  Prediction_all_rcs <- c()
  ...
  for (i in 1:10) {
    ...
    model <- glm.nb(as.formula(paste("Number_severe_asthma_attacks_during_followup ~ rcs(", 
                                     ...
                                     "+Age_imputated+Gender_imputated+BMI_imputated+ACQ_baseline_score_mean_imputated+FEV1_preBD_per10_Baseline_imputated+Eosinophils_Log_imputated+offset(log(Follow_up_duration_days)) + as.factor(Enrolled_Trial_name)")), 
                    data = subset(subgroup_data, .imp == i))
  ...
  Table_prediction_rcs <- cbind(subset(subgroup_data, .imp == i), prediction_combine_rcs)
  Table_prediction_rcs$Any_severe_attack_previous_12m_0no_1yes_imputated <- factor(Table_prediction_rcs$Any_severe_attack_previous_12m_0no_1yes_imputated, 
                                                                         levels = c("1", "0"))
  # Create the plot for the current subgroup
  plot <- ggplot(data = Table_prediction_rcs, 
                 aes(x = .data[[variable_1]], y = Prediction, colour = .data[[variable_2]])) +
    geom_smooth() +
    xlab('FeNO (ppb)') +
    ylab("Estimated annualised Severe Asthma Attack Rate") +
    scale_color_manual(labels = c("Yes", "No"), values = c("darkred", "darkgreen"), name = "Any attack past 12m") +
    scale_x_continuous(trans = 'log2', breaks = c(1, 10, 25, 50, 100, 200)) +
    scale_y_continuous(breaks = c(0.5, 1, 1.5)) +  # Custom y-axis ticks
    theme(legend.position = c(0.2, 0.8)) +
    theme(
    ...
    ggtitle(paste("Subgroup:", treatment)) +
    coord_cartesian(xlim = c(5, 220), ylim = c(0, 1.5))  # Adjust x and y axis limits here
  # Save the plot to the list
  plots_by_treatment[[treatment]] <- plot
}
# Display the plots
for (plot_name in names(plots_by_treatment)) {
  print(plots_by_treatment[[plot_name]])
}
Data_Oracle_only_with_asthma_history_yes_or_no$ICS_DOSE_CLASS_0_or_Low_combine

test<-Data_Oracle_only_with_asthma_history_yes_or_no %>% 
  filter(ICS_DOSE_CLASS=="0")

summary(test$Atopy_history_imputated)

# Spline curve Eos X Attack history according to the GINA STEP
variable_1<-"Blood_Eos_baseline_x10_9_cells_per_L_imputated"
variable_2<-"Any_severe_attack_previous_12m_0no_1yes_imputated"
Data_Oracle_only_with_asthma_history_yes_or_no$FeNO_Log_imputated
plots_by_treatment <- list()
...
for (treatment in unique(Data_Oracle_only_with_asthma_history_yes_or_no$Treatment_step_1and2)) {
  ...
  for (i in 1:10) {
    ...
    model <- glm.nb(as.formula(paste("Number_severe_asthma_attacks_during_followup ~ rcs(", 
                                     variable_1, ", 4)*", variable_2, 
                                     "+Age_imputated+Gender_imputated+BMI_imputated+ACQ_baseline_score_mean_imputated+FEV1_preBD_per10_Baseline_imputated+FeNO_Log_imputated+offset(log(Follow_up_duration_days)) + as.factor(Enrolled_Trial_name)")), 
                    data = subset(subgroup_data, .imp == i))
  ...
  plot <- ggplot(data = Table_prediction_rcs, 
    ...
    ggtitle(paste("Subgroup:", treatment)) +
    coord_cartesian(xlim = c(0.1, 1.5), ylim = c(0, 1.5))  # Adjust x and y axis limits here
  # Save the plot to the list
  plots_by_treatment[[treatment]] <- plot
}
# Display the plots
...
#======================================================================================================================================
# Spline curve FeNO X Attack history according to the ICS dose
Data_Oracle_only_with_asthma_history_yes_or_no$Eosinophils_Log_imputated
plots_by_treatment <- list()
variable_1<-"FeNO_baseline_ppb_imputated"
variable_2<-"Any_severe_attack_previous_12m_0no_1yes_imputated"
# Loop through each level of Treatment_step_1and2
for (treatment in unique(Data_Oracle_only_with_asthma_history_yes_or_no$ICS_DOSE_CLASS)) {
  # Filter the data for the current subgroup
  subgroup_data <- subset(Data_Oracle_only_with_asthma_history_yes_or_no, ICS_DOSE_CLASS == treatment)
  ...
  if (treatment=="0") {
    for (i in 1:10) {
      ...
      model <- glm.nb(as.formula(paste("Number_severe_asthma_attacks_during_followup ~ rcs(", 
                                       variable_1, ", 4)*", variable_2, 
                                       "+Age_imputated+Gender_imputated+BMI_imputated+ACQ_baseline_score_mean_imputated+FEV1_preBD_per10_Baseline_imputated+Eosinophils_Log_imputated+offset(log(Follow_up_duration_days))")), 
                      data = subset(subgroup_data, .imp == i))
...
# Spline curve BEC X Attack history according to the ICS dose
plots_by_treatment <- list()
variable_1<-"Blood_Eos_baseline_x10_9_cells_per_L_imputated"
variable_2<-"Any_severe_attack_previous_12m_0no_1yes_imputated"
# Loop through each level of Treatment_step_1and2
for (treatment in unique(Data_Oracle_only_with_asthma_history_yes_or_no$ICS_DOSE_CLASS)) {
  ...
  if (treatment=="0") {
    for (i in 1:10) {
      ...
      model <- glm.nb(as.formula(paste("Number_severe_asthma_attacks_during_followup ~ rcs(", 
                                       variable_1, ", 4)*", variable_2, 
                                       "+Age_imputated+Gender_imputated+BMI_imputated+ACQ_baseline_score_mean_imputated+FEV1_preBD_per10_Baseline_imputated+FeNO_Log_imputated+offset(log(Follow_up_duration_days))")), 
                      data = subset(subgroup_data, .imp == i))
...
Data_Table_1_Polyposis$CRSwNP_imputated
Data_Table_1_Polyposis$CRSsNP_imputated
...
Density_IgE_X_NP<-ggplot(subset(Data_Table_1_Polyposis, .imp == 1), aes(x = Total_IgE, fill = CRSwNP_imputated)) +
  ...
  scale_fill_manual(values = c("grey", "#E69F00"))  # Customize colors for each group

summary(Data_Table_1_Polyposis$Enrolled_Trial_name)
...
#Subgroup CRSwNP
CRSwNP_Yes_Group<-subset(Data_Table_1_Polyposis,CRSwNP_imputated=="Yes")
CRSwNP_Yes_Group$Age_by_group_imputated
CRSwNP_Yes_Group$Gender_imputated
CRSwNP_Yes_Group$BMI_by_group_imputated
CRSwNP_Yes_Group$ICS_DOSE_CLASS
CRSwNP_Yes_Group$ACQ5_by_group_imputated
CRSwNP_Yes_Group$Any_severe_attack_previous_12m_0no_1yes_imputated
CRSwNP_Yes_Group$FEV1_preBD_Baseline_by_group_imputated
CRSwNP_Yes_Group$FeNO_baseline_by_group_imputated
CRSwNP_Yes_Group$Eosinophils_by_group_imputated
...
#======================================================================================================================================
##Interaction Sex X attack history
filtered_trials <- unique(Data_Oracle$Enrolled_Trial_name[!is.na(Data_Oracle$Gender)&!is.na(Data_Oracle$Any_severe_attack_previous_12m_0no_1yes)])

Data_with_attack_history <- Data_Oracle %>%
  filter((Enrolled_Trial_name %in% filtered_trials))
Data_Oracle$Treatment_step_1and2

Female_Group<-subset(Data_with_attack_history,Gender_imputated=="Female")
Male_Group<-subset(Data_with_attack_history,Gender_imputated=="Male")

res_comb = NULL
for (i in 1:10){
  res_comb[[i]] =glm.nb(Number_severe_asthma_attacks_during_followup ~ Any_severe_attack_previous_12m_0no_1yes_imputated +BMI_per_5_imputated+Treatment_step_1and2+ACQ_baseline_score_mean_imputated+FEV1_preBD_per10_Baseline_imputated+ FeNO_Log_imputated + Eosinophils_Log_imputated +offset(log(Follow_up_duration_days)) + as.factor(Enrolled_Trial_name), data = subset(Female_Group, .imp == i))
}
res_pool <- summary(pool(res_comb), conf.int = TRUE,exp=TRUE)
Attack_Female<-res_pool[res_pool[,"term"]=="Any_severe_attack_previous_12m_0no_1yes_imputated",]

res_comb = NULL
for (i in 1:10){
  res_comb[[i]] =glm.nb(Number_severe_asthma_attacks_during_followup ~ Any_severe_attack_previous_12m_0no_1yes_imputated +BMI_per_5_imputated+Treatment_step_1and2+ACQ_baseline_score_mean_imputated+FEV1_preBD_per10_Baseline_imputated+ FeNO_Log_imputated + Eosinophils_Log_imputated +offset(log(Follow_up_duration_days)) + as.factor(Enrolled_Trial_name), data = subset(Male_Group, .imp == i))
}
res_pool <- summary(pool(res_comb), conf.int = TRUE,exp=TRUE)
Attack_Male<-res_pool[res_pool[,"term"]=="Any_severe_attack_previous_12m_0no_1yes_imputated",]
Attack_Male


Attack_Female_or_Male<-rbind(Attack_Female,Attack_Male)
rownames(Attack_Female_or_Male)<-c("Female","Male")

forplo(as.data.frame(Attack_Female_or_Male[,c("estimate","2.5 %","97.5 %")]),
       row.labels = c(paste("Female\n(n=",nrow(subset(Female_Group,.imp==1)),")",sep=""),paste("Male\n(n=",nrow(subset(Male_Group,.imp==1)),")",sep="")),
       em="aRR",
       left.align=FALSE,
       xlim= c(0.8,5),
       shade.every=1,
       ...
       margin.left=6,
       margin.right=10,
       add.columns=p_round(Attack_Female_or_Male[,"p.value"], digits = 2),
       add.colnames=c('p-value'),
       add.arrow.right=TRUE,
       arrow.right.length=10
)
...
#======================================================================================================================================
#ORACLE FINAL model proposal

Data_Oracle$Any_severe_attack_previous_12m_0no_1yes_imputated<-as.factor(Data_Oracle$Any_severe_attack_previous_12m_0no_1yes_imputated)
summary(Data_Oracle$Any_severe_attack_previous_12m_0no_1yes_imputated)

#Select data with attack history
Data_Oracle_with_attack_history<- Data_Oracle %>% 
  filter(Any_severe_attack_previous_12m_0no_1yes_imputated=="1")

#Select data with ICS DOSE low or medium or high
Data_Oracle_with_attack_history<- Data_Oracle_with_attack_history %>% 
  filter(ICS_DOSE_CLASS!="0")

#Create the category of attack history
Data_Oracle_with_attack_history <- Data_Oracle_with_attack_history %>%
  mutate(Attack_category_1_2 = case_when(
    Attack_12mo_Nb_imputated <= 1 ~ "1",      # Assign "1" when the condition is true
    Attack_12mo_Nb_imputated >= 2 ~ "2 or +"   # Assign "2 or +" for all other cases
  ))
Data_Oracle_with_attack_history$Attack_category_1_2<-as.factor(Data_Oracle_with_attack_history$Attack_category_1_2)

#Create the category of FEV1
Data_Oracle_with_attack_history$FEV1_preBD_PCT_Baseline_imputated
Data_Oracle_with_attack_history <- Data_Oracle_with_attack_history %>%
  mutate(FEV1_category_2 = case_when(
    FEV1_preBD_PCT_Baseline_imputated < 70 ~ "FEV1 <70%",     
    FEV1_preBD_PCT_Baseline_imputated >= 70 ~ "FEV1 >=70%"
  ))
Data_Oracle_with_attack_history$FEV1_category_2 <- factor(
  Data_Oracle_with_attack_history$FEV1_category_2,
  levels = c("FEV1 >=70%", "FEV1 <70%")  # Define the reference level
)

#Create the category of ACQ-5
Data_Oracle_with_attack_history$ACQ_baseline_score_mean_imputated
Data_Oracle_with_attack_history <- Data_Oracle_with_attack_history %>%
  mutate(ACQ_category_2 = case_when(
    ACQ_baseline_score_mean_imputated < 3 ~ "ACQ <3",     
    ACQ_baseline_score_mean_imputated >= 3 ~ "ACQ >=3"
  ))
Data_Oracle_with_attack_history$ACQ_category_2<- as.factor(Data_Oracle_with_attack_history$ACQ_category_2)
summary(Data_Oracle_with_attack_history$ACQ_category_2)

#Create the category of ICS_dose
Data_Oracle_with_attack_history$ICS_DOSE_CLASS
Data_Oracle_with_attack_history <- Data_Oracle_with_attack_history %>%
  mutate(ICS_category_3 = case_when(
    ICS_DOSE_CLASS == "Low" ~ "Low",     
    ICS_DOSE_CLASS == "Medium" ~ "Medium",
    ICS_DOSE_CLASS == "High" ~ "High"
  ))
Data_Oracle_with_attack_history$ICS_category_3 <- factor(
  Data_Oracle_with_attack_history$ICS_category_3,
  levels = c("Low", "Medium","High")  # Define the reference level
)

summary(Data_Oracle_with_attack_history$ICS_category_3)

#Create the category of BEC
Data_Oracle_with_attack_history$Blood_Eos_baseline_x10_9_cells_per_L_imputated
Data_Oracle_with_attack_history <- Data_Oracle_with_attack_history %>%
  mutate(BEC_category_3 = case_when(
    Blood_Eos_baseline_x10_9_cells_per_L_imputated < 0.15 ~ "BEC <0.15",     
    Blood_Eos_baseline_x10_9_cells_per_L_imputated < 0.3 ~ "BEC 0.15-0.3",  
    Blood_Eos_baseline_x10_9_cells_per_L_imputated >= 0.3 ~ "BEC >=0.3"
  ))
Data_Oracle_with_attack_history$BEC_category_3 <- factor(
  Data_Oracle_with_attack_history$BEC_category_3,
  levels = c("BEC <0.15", "BEC 0.15-0.3","BEC >=0.3")  # Define the reference level
)

summary(Data_Oracle_with_attack_history$BEC_category_3)

#Create the category of FeNO
Data_Oracle_with_attack_history$FeNO_baseline_ppb_imputated
Data_Oracle_with_attack_history <- Data_Oracle_with_attack_history %>%
  mutate(FeNO_category_3 = case_when(
    FeNO_baseline_ppb_imputated < 25 ~ "FeNO <25",     
    FeNO_baseline_ppb_imputated < 50 ~ "FeNO 25-<50",  
    FeNO_baseline_ppb_imputated >= 50 ~ "FeNO >=50"
  ))

Data_Oracle_with_attack_history$FeNO_category_3 <- factor(
  Data_Oracle_with_attack_history$FeNO_category_3,
  levels = c("FeNO <25", "FeNO 25-<50","FeNO >=50")  # Define the reference level
)
...
AICC_values<-c()
for (i in 1:10){
  res_comb[[i]] =glm.nb(Number_severe_asthma_attacks_during_followup ~ Gender_imputated+BMI_imputated+Attack_category_1_2+FEV1_category_2+FEV1_category_2+ACQ_category_2+ICS_category_3+BEC_category_3+FeNO_category_3+offset(log(Follow_up_duration_days)) + as.factor(Enrolled_Trial_name), data = subset(Data_Oracle_with_attack_history, .imp == i))
  AICC_values<-c(AICC_values,AICc(res_comb[[i]]))
  R2_values<-c(R2_values,1 - (summary(res_comb[[i]])$deviance / summary(res_comb[[i]])$null.deviance))
}
res_pool <- summary(pool(res_comb), conf.int = TRUE,exp=TRUE)
...
R2_mean<-mean(R2_values)
AICC_mean
(Data_Oracle_with_attack_history$Follow_up_duration_days)

# Step 1: Create a grid of all combinations of categorical variables
prediction_grid_w_attack <- expand.grid(
  FeNO_category_3 = levels(Data_Oracle_with_attack_history$FeNO_category_3),
  BEC_category_3 = levels(Data_Oracle_with_attack_history$BEC_category_3),
  ACQ_category_2 = levels(Data_Oracle_with_attack_history$ACQ_category_2),
  ICS_category_3 = levels(Data_Oracle_with_attack_history$ICS_category_3),
  FEV1_category_2 = levels(Data_Oracle_with_attack_history$FEV1_category_2),
  Attack_category_1_2 = levels(Data_Oracle_with_attack_history$Attack_category_1_2),
  
  
  
  
  
  Gender_imputated = "Female",
  BMI_imputated = mean(Data_Oracle_with_attack_history$BMI_imputated),
  Enrolled_Trial_name = "CAPTAIN",
  Follow_up_duration_days = 365  # Use mean follow-up duration
)
prediction_grid_w_attack
# Step 2: Predict for each imputation
predictions <- list()

for (i in 1:10) {
  # Predict based on the current model
  predictions[[i]] <- predict(
    res_comb[[i]],
    newdata = prediction_grid_w_attack,
    type = "response"
  )
}


# Step 3: Average predictions across imputations
prediction_grid_w_attack$Predicted_Risk <- rowMeans(do.call(cbind, predictions))
prediction_grid_w_attack$Predicted_Risk

write_xlsx(prediction_grid,"Prediction_ORACLE3.xlsx")



#Select data without attack history
Data_Oracle_without_attack_history<- Data_Oracle %>% 
  filter(Any_severe_attack_previous_12m_0no_1yes_imputated=="0")


#Create the category of FEV1
Data_Oracle_without_attack_history$FEV1_preBD_PCT_Baseline_imputated
Data_Oracle_without_attack_history <- Data_Oracle_without_attack_history %>%
  mutate(FEV1_category_2 = case_when(
    FEV1_preBD_PCT_Baseline_imputated < 70 ~ "FEV1 <70%",     
    FEV1_preBD_PCT_Baseline_imputated >= 70 ~ "FEV1 >=70%"
  ))

Data_Oracle_without_attack_history$FEV1_category_2 <- factor(
  Data_Oracle_without_attack_history$FEV1_category_2,
  levels = c("FEV1 >=70%", "FEV1 <70%")  # Define the reference level
)

#Create the category of ACQ-5
Data_Oracle_without_attack_history$ACQ_baseline_score_mean_imputated
Data_Oracle_without_attack_history <- Data_Oracle_without_attack_history %>%
  mutate(ACQ_category_2 = case_when(
    ACQ_baseline_score_mean_imputated < 3 ~ "ACQ <3",     
    ACQ_baseline_score_mean_imputated >= 3 ~ "ACQ >=3"
  ))
Data_Oracle_without_attack_history$ACQ_category_2<- as.factor(Data_Oracle_without_attack_history$ACQ_category_2)
summary(Data_Oracle_without_attack_history$ACQ_category_2)

#Create the category of ICS_dose
summary(Data_Oracle_without_attack_history$ICS_DOSE_CLASS)
Data_Oracle_without_attack_history <- Data_Oracle_without_attack_history %>%
  mutate(ICS_category_3 = case_when(
    ICS_DOSE_CLASS == "0" ~ "No",     
    ICS_DOSE_CLASS == "Low"|ICS_DOSE_CLASS == "Medium" ~ "Low-Medium",
    ICS_DOSE_CLASS == "High" ~ "High"
  ))
Data_Oracle_without_attack_history$ICS_category_3 <- factor(
  Data_Oracle_without_attack_history$ICS_category_3,
  levels = c("No", "Low-Medium","High")  # Define the reference level
)

summary(Data_Oracle_without_attack_history$ICS_category_3)

#Create the category of BEC
Data_Oracle_without_attack_history$Blood_Eos_baseline_x10_9_cells_per_L_imputated
Data_Oracle_without_attack_history <- Data_Oracle_without_attack_history %>%
  mutate(BEC_category_2 = case_when(
    Blood_Eos_baseline_x10_9_cells_per_L_imputated < 0.3 ~ "BEC <0.3",  
    Blood_Eos_baseline_x10_9_cells_per_L_imputated >= 0.3 ~ "BEC >=0.3"
  ))
Data_Oracle_without_attack_history$BEC_category_2 <- factor(
  Data_Oracle_without_attack_history$BEC_category_2,
  levels = c("BEC <0.3", "BEC >=0.3")  # Define the reference level
)

summary(Data_Oracle_without_attack_history$BEC_category_2)

#Create the category of FeNO
Data_Oracle_without_attack_history$FeNO_baseline_ppb_imputated
Data_Oracle_without_attack_history <- Data_Oracle_without_attack_history %>%
  mutate(FeNO_category_2 = case_when(
    FeNO_baseline_ppb_imputated < 50 ~ "FeNO <50",  
    FeNO_baseline_ppb_imputated >= 50 ~ "FeNO >=50"
  ))

Data_Oracle_without_attack_history$FeNO_category_2 <- factor(
  Data_Oracle_without_attack_history$FeNO_category_2,
  levels = c("FeNO <50","FeNO >=50")  # Define the reference level
)
...
AICC_values<-c()
for (i in 1:10){
  res_comb[[i]] =glm.nb(Number_severe_asthma_attacks_during_followup ~ Gender_imputated+BMI_imputated+FEV1_category_2+ACQ_category_2+ICS_category_3*FeNO_category_2+FeNO_category_2*BEC_category_2+offset(log(Follow_up_duration_days)) + as.factor(Enrolled_Trial_name), data = subset(Data_Oracle_without_attack_history, .imp == i))
  AICC_values<-c(AICC_values,AICc(res_comb[[i]]))
  R2_values<-c(R2_values,1 - (summary(res_comb[[i]])$deviance / summary(res_comb[[i]])$null.deviance))
}
res_pool <- summary(pool(res_comb), conf.int = TRUE,exp=TRUE)
...
R2_mean<-mean(R2_values)
AICC_mean
(Data_Oracle_without_attack_history$Follow_up_duration_days)

# Step 1: Create a grid of all combinations of categorical variables
prediction_grid_No_attack <- expand.grid(
  FeNO_category_2 = levels(Data_Oracle_without_attack_history$FeNO_category_2),
  BEC_category_2 = levels(Data_Oracle_without_attack_history$BEC_category_2),
  ACQ_category_2 = levels(Data_Oracle_without_attack_history$ACQ_category_2),
  ICS_category_3 = levels(Data_Oracle_without_attack_history$ICS_category_3),
  FEV1_category_2 = levels(Data_Oracle_without_attack_history$FEV1_category_2),
  Attack_category_1_2 = '0',
  
  
  Gender_imputated = "Female",
  BMI_imputated = mean(Data_Oracle_without_attack_history$BMI_imputated),
  Enrolled_Trial_name = "LAVOLTA_2",
  Follow_up_duration_days = 365  # Use mean follow-up duration
)
prediction_grid_No_attack
# Step 2: Predict for each imputation
predictions <- list()
...
for (i in 1:10) {
  ...
  predictions[[i]] <- predict(
    res_comb[[i]],
    newdata = prediction_grid_No_attack,
    type = "response"
  )
}
...
# Step 3: Average predictions across imputations
prediction_grid_No_attack$Predicted_Risk <- rowMeans(do.call(cbind, predictions))


colnames(prediction_grid_No_attack)<-colnames(prediction_grid_w_attack)
prediction_grid<-rbind(prediction_grid_w_attack,prediction_grid_No_attack)

write_xlsx(prediction_grid,"Prediction_ORACLE2_V1.xlsx")

#Create the category of ICS_dose
summary(Data_Oracle_without_attack_history$ICS_DOSE_CLASS)
Data_Oracle_without_attack_history <- Data_Oracle_without_attack_history %>%
  mutate(ICS_category_2 = case_when(
    ICS_DOSE_CLASS == "0"|ICS_DOSE_CLASS == "Low"|ICS_DOSE_CLASS == "Medium" ~ "Low-Medium",
    ICS_DOSE_CLASS == "High" ~ "High"
  ))
Data_Oracle_without_attack_history$ICS_category_2 <- factor(
  Data_Oracle_without_attack_history$ICS_category_2,
  levels = c("Low-Medium","High")  # Define the reference level
)
Data_Oracle_without_attack_history$ICS_category_2
...
AICC_values<-c()
for (i in 1:10){
  res_comb[[i]] =glm.nb(Number_severe_asthma_attacks_during_followup ~ Gender_imputated+BMI_imputated+FEV1_category_2+ACQ_category_2+ICS_category_2+FeNO_category_2*BEC_category_2+offset(log(Follow_up_duration_days)) + as.factor(Enrolled_Trial_name), data = subset(Data_Oracle_without_attack_history, .imp == i))
  AICC_values<-c(AICC_values,AICc(res_comb[[i]]))
  R2_values<-c(R2_values,1 - (summary(res_comb[[i]])$deviance / summary(res_comb[[i]])$null.deviance))
}
res_pool <- summary(pool(res_comb), conf.int = TRUE,exp=TRUE)
...
# Step 1: Create a grid of all combinations of categorical variables
prediction_grid_No_attack_v2 <- expand.grid(
  FeNO_category_2 = levels(Data_Oracle_without_attack_history$FeNO_category_2),
  BEC_category_2 = levels(Data_Oracle_without_attack_history$BEC_category_2),
  ACQ_category_2 = levels(Data_Oracle_without_attack_history$ACQ_category_2),
  ICS_category_2 = levels(Data_Oracle_without_attack_history$ICS_category_2),
  FEV1_category_2 = levels(Data_Oracle_without_attack_history$FEV1_category_2),
  Attack_category_1_2 = '0',
...
for (i in 1:10) {
  ...
  predictions[[i]] <- predict(
    res_comb[[i]],
    newdata = prediction_grid_No_attack_v2,
    type = "response"
  )
}
...
# Step 3: Average predictions across imputations
prediction_grid_No_attack_v2$Predicted_Risk <- rowMeans(do.call(cbind, predictions))


colnames(prediction_grid_No_attack_v2)<-colnames(prediction_grid_w_attack)
prediction_grid_v2<-rbind(prediction_grid_w_attack,prediction_grid_No_attack_v2)
prediction_grid_No_attack_v2
write_xlsx(prediction_grid_v2,"Prediction_ORACLE2_v2.xlsx")




#Creation of the model that integrate 
FEV1_intercept<-1.719505
FEV1_beta1 <- -0.00928251
FEV1_beta2 <-2.394093e-05
FEV1_beta3<--3.066601e-08
Tif_intercept<- 2.162559
Tif_beta1<--0.01650767
Tif_beta2<- 5.826234e-05
Tif_beta3<- -9.441995e-08

Calculator_OSI <- function(FEV1_PCT,Tif_PCT) {
  # Calculate OSI_final
  OSI <- 
    #calculation of the part related to FEV1
    ((FEV1_intercept + 
        FEV1_beta1 * FEV1_PCT + 
        FEV1_beta2 * FEV1_PCT^2 + 
        FEV1_beta3 * FEV1_PCT^3)-1) +
    #calculation of the part related to Tif
    ((Tif_intercept + 
        Tif_beta1 * (Tif_PCT) + 
        Tif_beta2 * (Tif_PCT)^2 + 
        Tif_beta3 * (Tif_PCT)^3)-1) +
    #adding the baseline risk
    1
...
#Calculation of FEV1 and FVC predicted in L
Data_Oracle$FEV1
Data_Oracle <- Data_Oracle %>% 
  mutate(FEV1_predicted_L= case_when(
    is.na(FEV1_predicted_L) ~ (FEV1_preBD_L_Baseline*100)/FEV1_preBD_PCT_Baseline, TRUE~FEV1_predicted_L )) %>% 
  mutate(FVC_predicted_L= case_when(
    is.na(FVC_predicted_L) ~ (FVC_preBD_L_Baseline*100)/FVC_preBD_PCT_Baseline, TRUE~FVC_predicted_L ))


#Calculation of Tif
Data_Oracle <- Data_Oracle %>% 
  mutate(Tif_preBD_Baseline= FEV1_preBD_L_Baseline/FVC_preBD_L_Baseline) %>% 
  mutate(Tif_postBD_Baseline= FEV1_postBD_L_Baseline/FVC_postBD_L_Baseline)

#Calculation of Tif_predicted
Data_Oracle <- Data_Oracle %>% 
  mutate(Tif_predicted=FEV1_predicted_L/FVC_predicted_L)
summary(Data_Oracle$Tif_predicted)

summary(Data_Oracle$FEV1_FVC_ratio_imputated/(Data_Oracle$FEV1_predicted_L/Data_Oracle$FVC_predicted_L))
Data_Oracle$Tiffeneau_imputated
summary(Data_Oracle$FEV1_predicted_L/Data_Oracle$FVC_predicted_L)
Calculator_OLI(Data_Oracle$FEV1_preBD_PCT_Baseline_imputated,Data_Oracle$FEV1_preBD_PCT_Baseline_imputated)
